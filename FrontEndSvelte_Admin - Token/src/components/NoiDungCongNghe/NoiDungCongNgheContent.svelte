<script lang="ts">
  ...
  async function fetchItems() {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/congnghe/search?${query}`
      );
  ...
  function toggleSort(key: keyof CongNghe) {
    if (sortKey === key) {
  ...
  if (userRole === "SuperAdmin" || userRole === "Admin") {
    if (confirm("Are you sure you want to delete this item?")) {
      await fetch(`http://localhost:3000/api/congnghe/delete/${itemId}`, {
        method: "DELETE",
      });
      fetchItems();
      location.reload();
  ...
  checkboxes.forEach((checkbox: Element) => {
    const inputCheckbox = checkbox as HTMLInputElement; // Cast Element to HTMLInputElement
    inputCheckbox.checked = isChecked; // Cập nhật trạng thái của checkbox
    const row = inputCheckbox.closest("tr"); // Lấy hàng <tr> chứa checkbox
    ...
    if (userRole === "SuperAdmin" || userRole === "Admin") {
      ...
      if (confirm("Are you sure you want to delete the selected items?")) {
        ...
        try {
          ...
          await fetch(`http://localhost:3000/api/congnghe/delete`, {
            method: "DELETE",
          ...
          <tr>
            ...
            <td>
              <button
                ...
                on:click={() => openModal(String(item.ID))}  
                style="color: white;"
              >
                View
              </button>
            ...
            <td>
              <button
                ...
                on:click={() => editItem(String(item.ID))}
                style="color: white;"
              >
                Edit
              </button>
              <button
                ...
                on:click={() => deleteItem(String(item.ID))}
              >
                Delete
              </button>
            </td>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            {#if showModal !== null && data.find((item) => item.ID === Number(showModal))}
              <div class="info-row">
                <p>
                  <strong>ID:</strong>
                  {data.find((item) => item.ID === (showModal ? Number(showModal) : NaN))?.ID}
                </p>
                <p>
                  <strong>Name:</strong>
                  {data.find((item) => item.ID === Number(showModal))?.Ten}
                </p>
                <p>
                  <strong>Order:</strong>
                  {data.find((item) => item.ID === Number(showModal))?.STT}

                </p>
                <p>
                  <strong>Status:</strong>
                  {#if data.find((item) => item.ID === Number(showModal))?.TrangThai === 1}
                    <span class="badge bg-success">Active</span>
                  {:else if data.find((item) => item.ID === Number(showModal))?.TrangThai === 0}
                    <span class="badge bg-danger">Locked</span>
                  {:else}
              ...
              <p>
                ...
                {data.find((item) => item.ID === Number(showModal))?.TieuDe}
              </p>
              <p>
                <strong>Description:</strong>
                {data.find((item) => item.ID === Number(showModal))?.MoTa}
              </p>
              <p>
                <strong>Meta Keyword:</strong>
                {data.find((item) => item.ID === Number(showModal))?.MetaKeyword}
              </p>
              <p>
                <strong>Meta Description:</strong>
                {data.find((item) => item.ID === Number(showModal))?.MetaDescription}
              </p>
              ...
              <div class="info-row">
                <p>
                  ...
                  {#if showModal !== null}
                    {formatDate(data.find((item) => item.ID === Number(showModal))?.NgayTao)} by
                    {data.find((item) => item.ID === Number(showModal))?.NguoiTao}
                  {/if}
                </p>
                <p>
                  <strong>Updated:</strong>
                  {#if showModal !== null}
                    {formatDate(data.find((item) => item.ID === Number(showModal))?.NgayCapNhat)} by
                    {data.find((item) => item.ID === Number(showModal))?.NguoiCapNhat}
                {/if}
                </p>
              </div>
              ...
              <p>
                ...
                {data.find((item) => item.ID === Number(showModal))?.LoaiCongNgheID}
              </p>
              <p>
                <strong>Show on homepage:</strong>{" "}
                {#if showModal !== null}
                  {#if data.find((item) => item.ID === Number(showModal))?.HienThiTrangChu === 1}
                    <span class="badge bg-success">True</span>
                  {:else if data.find((item) => item.ID === Number(showModal))?.HienThiTrangChu === 0}
                    <span class="badge bg-danger">False</span>
                  {:else}
                    <span class="badge bg-secondary">Unknown</span>
                  {/if}
                  <strong style="margin-left: 20px;">Show on hot:</strong>{" "}
                  {#if data.find((item) => item.ID === Number(showModal))?.HienThiHot === 1}
                    <span class="badge bg-success">True</span>
                  {:else if data.find((item) => item.ID === Number(showModal))?.HienThiHot === 0}
                    <span class="badge bg-danger">False</span>
                  {:else}
                    <span class="badge bg-secondary">Unknown</span>
                  {/if}
