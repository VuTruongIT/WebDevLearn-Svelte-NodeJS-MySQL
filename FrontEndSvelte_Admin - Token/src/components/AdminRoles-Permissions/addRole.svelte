<script lang="ts">
  ...
  async function addVaiTro() {
    ...
    try {
      const response = await fetch("http://localhost:3000/api/vaitro/add", {
        method: "POST",
      ...
      goto("/admin/roles-permissions"); // Chuyển trang sau khi tạo vai trò thành công
    } catch (error) {
  ...
  async function saveAndAddAnother() {
    ...
    try {
      ...
      // Reset các trường đầu vào để người dùng tạo vai trò mới
      tenVaiTro = "";
      moTa = "";
    } catch (error) {
      alert("Error creating role...");
...
<div class="content">
  ...
  <form on:submit|preventDefault={addVaiTro} class="form-container">
    ...
    <div class="form-actions">
      ...
      <button type="button" class="btn btn-primary" on:click={saveAndAddAnother}>Save and Add Another</button>
